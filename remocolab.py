...
def _setupSSHDImpl(ngrok_token, ngrok_region, ipython_output):
  ...
  if ipython_output:
    ...
    display(HTML(f"<p>root password: <input value='{root_password}' size='{len(root_password)+5}' type='text' style='font-family:monospace' readonly/></p>"))
    display(HTML(f"<p>{user_name} password: <input value='{user_password}' size='{len(user_password)+5}' type='text' style='font-family:monospace' readonly/></p>"))
  else:
  ...
  if ipython_output:
    display(HTML(f"<p><input value='{cmd_string}' size='{len(cmd_string)+5}' type='text' style='font-family:monospace' readonly/></p>"))
  else:
    print(cmd_string)
  print("✂️"*24)
  print("---")
