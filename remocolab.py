...
def _setupSSHDImpl(ngrok_token, ngrok_region, ipython_output):
  ...
  if ipython_output:
    ...
    display(HTML(f"<p>root password: <input value='{root_password}' size='{len(root_password)}' type='text' style='font-family:monospace' readonly/></p>"))
    display(HTML(f"<p>{user_name} password: <input value='{user_password}' size='{len(user_password)}' type='text' style='font-family:monospace' readonly/></p>"))
  else:
  ...
  if ipython_output:
    display(HTML(f"<p><input value='{cmd_string}' size='{len(cmd_string)}' type='text' style='font-family:monospace' readonly/></p>"))
  else:
    print(cmd_string)
  print("✂️"*24)
  print("---")
...
if ipython_output:
  from IPython.core.display import display, HTML
  display(HTML(f"<p>VNC password: <input value='{vnc_passwd}' size='{len(vnc_passwd)}' type='text' style='font-family:monospace' readonly/></p>"))
  display(HTML(f"<p>VNC view only password: <input value='{vnc_viewonly_passwd}' size='{len(vnc_viewonly_passwd)}' type='text' style='font-family:monospace' readonly/></p>"))
else:
